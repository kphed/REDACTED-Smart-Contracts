...
import { REDACTEDTreasury, IERC20, REDACTEDLPBondDepositoryRewardBased } from '../typechain'
...
describe('Live LP bonds', function () {
  beforeEach(async function () {
    ...
    if (lpTokenFloor.eq(BigNumber.from(0))) {
      await treasuryContract.setFloor(LP_TOKEN_ADDRESS, BigNumber.from('1000000000'))
    }
    ...
    await treasuryContract.queue(BigNumber.from(5), LP_TOKEN_ADDRESS)

    await treasuryContract.toggle(BigNumber.from(5), LP_TOKEN_ADDRESS, BONDING_CALCULATOR)
    ...
    await treasuryContract.queue(BigNumber.from(4), lpBond.address)

    await treasuryContract.toggle(BigNumber.from(4), lpBond.address, ZERO_ADDRESS)
  })
  ...
  it('MinPrice of 8500 gives [Zeus] a discount between 12% & 15% out the gate', async function () {
    ...
    await lpToken.connect(lpWhale).approve(lpBond.address, ethers.constants.MaxUint256)

    const lpBtrflyBalance = await btrfly.balanceOf(LP_TOKEN_ADDRESS)

    const lpSupply = await lpToken.totalSupply()

    const lpWhaleDepositBtrflyValue = BigNumber.from(2)
      .mul(lpBtrflyBalance)
      .mul(ethers.utils.parseUnits('0.1', 'gwei'))
      .div(lpSupply)

    const redemptionMinValue = lpWhaleDepositBtrflyValue
      .mul(BigNumber.from(112))
      .div(BigNumber.from(100))

    const redemptionMaxValue = lpWhaleDepositBtrflyValue
      .mul(BigNumber.from(118))
      .div(BigNumber.from(100))
    ...
    await lpBond
      .connect(lpWhale)
      .deposit(ethers.utils.parseUnits('0.1', 'gwei'), BigNumber.from(10000), recipient.address)
